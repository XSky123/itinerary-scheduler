...
import type { TransitOption, Timeline, TransitType } from '../lib/models'
...
interface ContextMenu {
  ...
  x: number
  y: number
  fromPlanId?: string  // set when right-clicking a plan lane block
}
...
export default function TimelineEditor() {
  ...
  const dragParamsRef = useRef({ totalMs: 0, updateTransit })
  ...
  useEffect(() => {
    const onMove = (e: MouseEvent) => {
      ...
      const { totalMs: ms, updateTransit: upd } = dragParamsRef.current
    ...
    const onUp = () => {
      // Only push history if the user actually dragged (not just clicked)
      if (dragRef.current && dragMovedRef.current && preDragStateRef.current) {
  ...
  const openCtx = (e: React.MouseEvent, transitId: string, fromPlanId?: string) => {
    e.preventDefault(); e.stopPropagation()
    setContextMenu({ transitId, x: e.clientX, y: e.clientY, fromPlanId })
  }
  ...
  const startPlanEdit = (tl: Timeline) => {
    setEditingPlanId(tl.id); setEditingPlanName(tl.name)
  }
  ...
  return (
    <main className="panel panel-editor">
                    ...
                    <div className="gantt-row-content">
                      ...
                      {planTransits.map(transit => {
                        ...
                        return (
                          ...
                          >
                            {/* (2) 3-line: row name / transit name (optional) / time */}
      ...
      <div className="plan-footer">
        <span className="plan-footer-hint">点击块在左栏编辑 · 拖拽移动 · 右键管理计划 · Ctrl+Z 撤销</span>
        {selectedTimelineId && (
        ...
        <>
          ...
          <div className="ctx-menu" style={{ left: contextMenu.x, top: contextMenu.y }} onClick={e => e.stopPropagation()}>
            {contextMenu.fromPlanId ? (
              /* Right-clicked a plan lane block → direct remove */
              <>
                <div className="ctx-header">移出计划</div>
                <button className="ctx-item" onClick={() => removeFromPlan(contextMenu.fromPlanId!, contextMenu.transitId)}>
                ...
                {timelines.filter(t => t.id !== contextMenu.fromPlanId).map((tl, i) => {
                  ...
                  const isIn = tl.segments.some(s => s.transitId === contextMenu.transitId)
                  if (!isIn) return null
                  return (
                    <button key={tl.id} className="ctx-item ctx-item-in" onClick={() => removeFromPlan(tl.id, contextMenu.transitId)}>
              ...
              /* Right-clicked a source block → toggle menu */
              timelines.length === 0 ? <div className="ctx-header">请先新建计划</div> : (
                <>
                  ...
                  {timelines.map((tl, idx) => {
                    const isIn = tl.segments.some(s => s.transitId === contextMenu.transitId)
                    return (
                      <button key={tl.id} className={`ctx-item${isIn ? ' ctx-item-in' : ''}`} onClick={() => toggleInPlan(tl.id, contextMenu.transitId)}>
